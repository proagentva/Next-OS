...
import type { SocialPost, SocialPostStatus, Profile } from '../lib/types'
import { Avatar } from '../components/Avatar'
import { formatDate } from '../lib/utils'
import { Plus, Check, RotateCcw, X, ExternalLink } from 'lucide-react'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {visiblePosts.map(post => (
            <div key={post.id} className="card p-4 space-y-3">
              ...
              {post.caption && <p className="text-sm text-ink-700 dark:text-ink-300 whitespace-pre-wrap">{post.caption}</p>}
              {post.link && (
                <a href={post.link} target="_blank" rel="noopener noreferrer" className="text-sm text-accent-600 dark:text-accent-400 hover:underline inline-flex items-center gap-1 break-all">
              ...
              {isAdmin && tab === 'pending' && (
                <div className="flex gap-2 pt-1">
                  <button onClick={() => review(post, 'approved')} className="btn-secondary text-xs flex-1 justify-center text-accent-700 dark:text-accent-400">
                    <Check size={13} /> Approve
                  </button>
                  <button onClick={() => review(post, 'revise')} className="btn-secondary text-xs flex-1 justify-center text-gold-700 dark:text-gold-400">
                    <RotateCcw size={13} /> Revise
                  </button>
                  <button onClick={() => review(post, 'disapproved')} className="btn-secondary text-xs flex-1 justify-center text-red-600 dark:text-red-400">
                    <X size={13} /> Disapprove
                  </button>
                </div>
              )}
            </div>
